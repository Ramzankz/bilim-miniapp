...
export default function ParentDashboard({ lang, t, onBack }) {
  ...
  const avgPct = totalLessons
    ? Math.round(stats.reduce((a, s) => a + s.score / s.total, 0) / totalLessons * 100)
    : 0;

  const topicMap = {};
  stats.forEach(s => {
    if (!topicMap[s.title]) topicMap[s.title] = { correct: 0, total: 0, count: 0 };
    topicMap[s.title].correct += s.score;
    topicMap[s.title].total  += s.total;
    topicMap[s.title].count  += 1;
  });
  ...
  if (!unlocked) return (
    <div className="screen parent-screen">
      ...
      <div className="pin-wrap">
        ...
        <h2>{t("Ата-ана кабинеті", "Кабинет родителя")}</h2>
        <p className="pin-hint">PIN: <b>1234</b></p>
  ...
  return (
    <div className="screen parent-screen">
      <div className="parent-header">
        <button className="back-btn" onClick={onBack}>✕</button>
        <h2>{t("📊 Статистика", "📊 Статистика")}</h2>
      </div>

      <div className="pstat-grid">
        {[
          { icon: "📚", val: totalLessons, label: t("Сабақ өтті","Уроков пройдено") },
          { icon: "🎯", val: avgPct+"%",   label: t("Орташа нәтиже","Средний балл") },
          { icon: "🪙", val: coins,         label: t("Тиын","Монет") },
          { icon: "🔥", val: streak,        label: t("Күн қатарынан","Дней подряд") },
        ].map((s,i) => (
      ...
      {topics.length > 0 ? (
        <div className="topic-list">
          <h3>{t("Тақырыптар бойынша нәтиже:","Результаты по темам:")}</h3>
          {topics.map((tp, i) => (
        ...
        <div className="no-stats">
          <p>🧒 {t("Бала әлі сабақ өтпеген","Ребёнок ещё не проходил уроки")}</p>
        </div>
      )}
    </div>
  );
